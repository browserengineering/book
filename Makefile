...
www/%.html: book/%.md book/template.html book/signup.html book/filter.lua disabled.conf
	pandoc --template book/template.html $(FLAGS) -c book.css --variable=script:feedback.js --from markdown --to html --lua-filter=book/filter.lua $< -o $@
...
www/draft/%.html: book/%.md book/template.html book/signup.html book/filter.lua
	pandoc --template book/template.html $(FLAGS) --metadata=mode:draft --variable=script:../feedback.js -c ../book.css --from markdown --to html --lua-filter=book/filter.lua $< -o $@
