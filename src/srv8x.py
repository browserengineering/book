...
def handle_request(method, url, headers, body):
    if method == 'POST':
        ...
        if 'guest' in params and 'add' in params and len(params) <= 100:
            ENTRIES.setdefault(url, []).append(params['guest'])

    out = '<!doctype html>'
    out += "<body>"
    out += "<form action={} method=post><p><input name=guest /></p><p><button>Sign the book!</button></p></form>".format(url)
