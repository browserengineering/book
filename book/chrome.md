...
Note that `layout_block` and `layout_inline` now both recurse into the
children and compute height in the same way, so we can move that code
back into `layout`. And once we've done that, `layout_block` and
`layout_inline` are both pretty short, so we can inline them back into
`layout`:
...
bit simpler and more regular.

You might also want to delete the `flush` method, since it's no longer
called from anywhere. But keep it around for just a moment---we'll
need it to write the `layout` method for line and text objects.
