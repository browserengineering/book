...
Similarly, `AlphaAndBlend` should indicate which parameters were
actually passed:
...
to a Skia surface created by `AlphaAndBlend`, and then copying that to the root
...
frames is the `AlphaAndBlend`, which is in the draw display list.
...
`AlphaAndBlend(alpha=0.5)` then right now there might be two surface copies:
